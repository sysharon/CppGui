...
{
	for (bool redraw = true; isEventEngineRunning;)
	{
		if (redraw)
		{
		...
		case MOUSE_EVENT:
		{
			...
			if (button == FROM_LEFT_1ST_BUTTON_PRESSED || button == RIGHTMOST_BUTTON_PRESSED)
			{
				...
				{
					c.onMouseClick(x, y, button == FROM_LEFT_1ST_BUTTON_PRESSED);
					redraw = true;
				}
			}
			break;
...
void EventEngine::moveFocusForword(Control& main, Control* focused)
{
	...
	while (!(*it)->enableFocus() || !(*it)->isVisible()
...
EventEngine& EventEngine::getEngine() {
	if (!eventEngine) eventEngine = new EventEngine;
	return *eventEngine;
}