...
void Panel::onKeyDown(const WORD code, const CHAR chr) {
	try {
		switch (code) {
	...
	catch (exception& MsgBox) { perror("undefined key"); }
}

void Panel::onMouseClick(const int x, const int y, const bool isLeft) {
	if (isLeft) {
		for (auto it = children.begin(); it != children.end(); ++it) {
			int X = x - (*it)->getLeft();
			int Y = y - (*it)->getTop();
			if (X >= 0 && Y >= 0 && X < (*it)->getWidth() && Y < (*it)->getHeight()
				...
				if ((*it)->enableFocus()) setFocus(**it);
				(*it)->onMouseClick(X, Y, isLeft);
				break;
			}
		}
	}
